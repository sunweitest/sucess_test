...
# dir = path.dirname(path.dirname(path.dirname(path.dirname(file_name))))

# here = os.path.abspath(os.path.dirname(__file__))
# UPLOAD_FOLDER = path.join(here,'upload')
UPLOAD_FOLDER = path.join('upload/')
...
def delete_records(keep=20):
    ...
    # db.session.query(Mind).from_statement(
    #     text('SELECT * from mind where is_deleted != 1 ORDER BY id desc LIMIT -1 offset 20'))
...
def get_records(limit=8):
    ...
    # rows = Mind.query.filter(Mind.is_deleted != 1).all()
...
def preview_file(filename):
    ...
    full_path = path.join(UPLOAD_FOLDER,filename)
    print(full_path)
    # full_path = path.join(r'D:\BaiduNetdiskDownload\smart_test\upload',filename)
    # full_path = r'D:\BaiduNetdiskDownload\smart_test\upload\XMind测试用例模板'
